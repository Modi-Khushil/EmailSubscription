...
public class UserDao {
	...
	public int subscribeUser(UserBean userBean) {
		try {
			userBean = stmt.queryForObject("SELECT * FROM user WHERE email = ? AND is_active = true",
			...
			try {
				int count = stmt.update("INSERT INTO user (email) VALUES (?)", userBean.getEmail());
				if (count > 0) {
					userBean.setUser_id(
							stmt.queryForObject("SELECT currval('categuser')", new RowMapper<Integer>() {
								@Override
					...
					if (count > 0) {
						mailSender.sendMail(userBean.getEmail(),
								"Successfully subscribed to Khushil Modi's Newsletter",
