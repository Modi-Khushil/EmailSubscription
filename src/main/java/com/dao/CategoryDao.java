...
public class CategoryDao {
	...
	public List<CategoryBean> fetchCategories() {
		try {
			return stmt.query("SELECT * FROM category WHERE is_active = true", new RowMapper<CategoryBean>() {
				@Override
	...
	public int subscribeCategories(UserBean userBean) {
		try {
			for (CategoryBean categoryBean : userBean.getCategories()) {
				stmt.update("INSERT INTO categuser (user_id, category_id) VALUES (?, ?)", userBean.getUser_id(), categoryBean.getCategory_id());
			}			
			return 1;
		} catch (DataAccessException e) {
			return 0;
