...
public class TopicDao {
	...
	public List<UserBean> getTopicSubscribers(TopicBean topicBean) {
		...
		try {
			if (category_id != 0) {
				return stmt.query(
						"SELECT * FROM user u JOIN categuser c ON u.user_id = c.user_id WHERE c.category_id = ? AND c.is_active = true",
				...
				return stmt.query("SELECT * FROM user WHERE is_active = true", new RowMapper<UserBean>() {
					@Override
					public UserBean mapRow(ResultSet rs, int rowNum) throws SQLException {
						UserBean userBean = new UserBean();
						userBean.setUser_id(rs.getInt("user_id"));
